...
Shader "QuikFX/VFX"
{
    Properties
    {
		...
		_ScrollingSpeedGrad("Scrolling speed Gradient", Range(-30, 30)) = 0
		...
		_DisplacementStrength("Displacement Strength", Range(0,10)) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {		
				...
				float speedMul = _SpeedMultiplier + (i.customData.y * _UseCustomData);
				...
				float4 secondaryTex = float4(1,1,1,1);
					...
					float4 distortionTex = tex2D(_DistortionTex, uvDistortion);
					distort = float2((distortionTex.x - 0.5) * _DistortionStrengthX, (distortionTex.y - 0.5) * _DistortionStrengthY);
				#endif

				float2 uvA = (pixelUv * _TilingSpeedA.xy + _Time.y * _TilingSpeedA.zw * speedMul) + distort ;
				...
				float4 col = mainTex;
					...
					float2 uvB = (pixelUv * _TilingSpeedB.xy + _Time.y * _TilingSpeedB.zw * speedMul) + distort;
