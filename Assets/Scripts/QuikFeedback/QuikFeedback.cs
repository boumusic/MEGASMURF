...
namespace Pataya.QuikFeedback
{
    ...
    public class ZoomFeedback : Feedback
    {
        ...
        [Range(0f, 20f)] public float duration = 0.15f;
    ...
    public class PostProcessFeedback : Feedback
    {
        ...
        [Range(0f, 20f)] public float duration = 0.2f;
    }
    ...
    public class MaterialPropertyFeedback
    {
        ...
        [Header("Values")]
        [Range(0.001f, 20f)] public float duration = 0.5f;
    ...
    #region GamepadRumble

    [System.Serializable]
    public class RumbleFeedback : Feedback
    {
        [Header("Rumble settings")]
        [Range(0, 3)] public float duration = 1f;
        [Range(0, 5)] public float leftSpeed = 0.5f;
        public AnimationCurve leftCurve = new AnimationCurve(new Keyframe[] { new Keyframe(0f, 0f), new Keyframe(0.5f, 1f), new Keyframe(1f, 0f) });

        [Range(0, 5)] public float rightSpeed = 1f;
        public AnimationCurve rightCurve = new AnimationCurve(new Keyframe[] { new Keyframe(0f, 0f), new Keyframe(0.5f, 1f), new Keyframe(1f, 0f) });
    ...
    public class QuikFeedback : MonoBehaviour
    {
        ...
        public RumbleFeedback rumbleFeedback;

        public bool useGameobject = false;
        public GameObjectFeedback gameObjectFeedback;
        ...
        public void Play()
        {
            ...
            {
                if (QuikFeedbackManager.instance != null)
        ...
        private IEnumerator PlayingFeedback()
        {
            ...
            if (useRumble)
            {
                StartCoroutine(PlayingRumble());
            }

            if (useGameobject)
            {
                StartCoroutine(PlayingGameObject());
            }
        }
        ...
        private IEnumerator PlayingAnimations()
        {
            ...
            for (int i = 0; i < animationFeedback.animations.Length; i++)
            {
                ...
                if (anim.parameterType == AnimationParameterType.Bool)
                {
                    ...
                    if (anim.resets)
                    {
                        yield return new WaitForSeconds(anim.resetTime);
                        animationFeedback.animations[i].anim.SetBool(param, !anim.boolValue);
                    }
                }
                ...
                {
                    animationFeedback.animations[i].anim.ResetTrigger(param);
        ...
        private IEnumerator PlayingRumble()
        {
            yield return new WaitForSeconds(rumbleFeedback.delay);
            QuikFeedbackManager.instance.Rumble(rumbleFeedback);
        }

        private IEnumerator PlayingGameObject()
        {
            yield return new WaitForSeconds(gameObjectFeedback.delay);
            for (int i = 0; i < gameObjectFeedback.gameObjects.Length; i++)
            {
                gameObjectFeedback.gameObjects[i].SetActive(gameObjectFeedback.setActive);
            }
        }
