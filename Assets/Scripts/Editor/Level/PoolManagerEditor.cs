...
public class PoolManagerEditor : Editor
{
    ...
    private void DrawSinglePool(int i, SerializedProperty pools)
    {
        if (i < m.pools.Count)
        {
            ...
            bool hasEntity = true;
            LevelElement elem = null;
            if (prefab) elem = prefab.GetComponentInChildren<LevelElement>();


            if (m.pools[i].isLevelElement)
            {
                if (prefab)
                {
                    if (elem)
                    {
                        Color col = elem.ColorInEditor();
                        GUI.color = col;
                    }
                }
            }
                ...
                if (m.pools[i].foldout)
                {
                    ...
                    if (hasPrefab)
                    {
                        ...
                        else
                        {
                            ...
                            GUI.enabled = false;
                            m.pools[i].isLevelElement = m.pools[i].prefab.GetComponentInChildren<LevelElement>();
                            CustomEditorUtility.QuickSerializeRelative("isLevelElement", arrayElement);
